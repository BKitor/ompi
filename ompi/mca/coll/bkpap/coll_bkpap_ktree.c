...
int coll_bkpap_papaware_ktree_allreduce_fullpipelined(const void* sbuf, void* rbuf, int count,
    ...
    if (is_inter) { // do PAP-Aware stuff
        ...
        for (int seg_index = 0; seg_index < num_segments; seg_index++) {
            ...
            ret = bk_ompi_request_wait_all(tmp_bcast_wait_arr, 3);
        ...
        ret = bk_ompi_request_wait_all(intra_bcast_reqs, 2);
        BKPAP_CHK_MPI_MSG_LBL(ret, "leader bk_request_wait_all failed", bkpap_fullpipe_allreduce_exit);
...
int coll_bkpap_papaware_ktree_allreduce_pipelined(const void* sbuf, void* rbuf, int count,
    ...
    if (is_inter) { // Node leader, do pipelineing and pap-awareness
        ...
        for (int seg_index = 0; seg_index < num_segments; seg_index++) {
            ...
            tmp_bcast_wait_arr[1] = inter_bcast_reqs[phase_selector];
            tmp_bcast_wait_arr[2] = ss_reset_barrier_reqs[phase_selector];
            bk_ompi_request_wait_all(tmp_bcast_wait_arr, 3);
...
int coll_bkpap_papaware_ktree_allreduce(const void* sbuf, void* rbuf, int count,
