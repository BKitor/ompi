    ...
    if (is_inter) {
        ...
        for (int seg_index = 0; seg_index < num_segments; seg_index++) {
            ...
            if (seg_index > 1) { // Launch Intra-bcast
                BKPAP_OUTPUT("STARTING_INTRA_IBCAST: seg_index:%d, rank: %d, arrival: %ld", seg_index, mpi_rank, arrival_pos);
            ...
            int sync_mask = 1;
            for (int sync_round = 0; sync_round < remote_ss_tmp->ss_counter_len; sync_round++) { // pap-aware loop
                ...
                if (-1 == arrival_pos) {
                    ...
                    BKPAP_OUTPUT("ARRIVED_AT_SS: seg_index: %d, rank: %d, arrive: %ld", seg_index, mpi_rank, arrival_pos);
                ...
                if (0 == arrival_pos % sync_mask) { // if-parent reduce
                    ...
                    BKPAP_OUTPUT("LEAVE_REDUCE: seg_index: %d, rank %d, seg_buf[0]: %.2f", seg_index, mpi_rank, ((float*)seg_buf)[0]);
                } // if-parent reduce
...
static inline int _bk_papaware_ktree_allreduce(const void* sbuf, void* rbuf, int count,
    ...
    if (ompi_comm_rank(comm) == tree_root) {
        ...
        _BK_CHK_RET(ret, "reset_remote_ss failed");
        BKPAP_PROFILE("rank: %d, reset_remote_ss", mpi_rank);
    }
